...
=end
	...
	describe ".create_note" do
		...
		context "For all modules, with valid note content and a valid record", :focus => true do
			it "should return true and Hash containing note_id vs note_obj" do
				...
				list.each do |mod|
					...
					bool, note_hash = r_obj.create_note(note_title, note_content)
					expect(bool).to eq true
					expect(note_hash).not_to be_nil
					expect(note_hash).not_to be_empty
					expect(note_hash).to be_instance_of(Hash)
					n_id, n_obj = nil, nil
					note_hash.each do |i, n|
						n_id = i
						n_obj = n
					end
					expect(n_id).not_to be_nil
